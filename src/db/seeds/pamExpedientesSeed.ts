...
export async function seedPamExpedientes() {
  ...
  const estados = ["Pendiente", "En Circulación", "Resuelto", "Con Lugar", "Sin Lugar", "Parcial", "Caducado", "Dictamen", "Requerido"];
  ...
  for (let semanaId = 1; semanaId <= 12; semanaId++) {
    ...
    for (let i = 0; i < expedientesPorSemana; i++) {
      ...
      fechaUltimaModificacion.setDate(fechaIngreso.getDate() + Math.floor(Math.random() * 5)); // Modificación 0-5 días después
      ...
      expedientes.push({
        ...
        estado,
        fechaUltimaModificacion: fechaUltimaModificacion.toISOString().split('T')[0],
  ...
  try {
    ...
    console.log(`✅ Se insertaron ${expedientes.length} expedientes exitosamente`);
  } catch (error) {
